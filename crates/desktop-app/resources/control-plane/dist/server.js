...
function resolveExportRoot(artifactRoot) {
...
function buildLiveViewPath(sessionId) {
    return `/api/sessions/${sessionId}/live-view/`;
}
function deriveLiveDesktopView(session) {
...
function deriveReviewRecording(session) {
...
function withLiveDesktopView(session) {
...
function buildLiveViewProxyPath(pathname, sessionId) {
...
function rewriteLiveViewBody(sessionId, proxyPath, contentType, body) {
    ...
    if (contentType?.includes('text/html') && ['/', '/index.html', '/vnc.html'].includes(proxyPath)) {
        return Buffer.from(body.toString('utf8').replace(/value="websockify"/g, `value="${websocketPath}"`), 'utf8');
    }
...
function copyProxyResponseHeaders(source, target) {
...
export function toGuestAction(action) {
...
async function fetchBrowserSnapshot(url) {
    try {
...
function getDesktopActivateArgs() {
...
async function maybeActivateDesktop(session) {
    ...
    if (!liveDesktopView || liveDesktopView.mode !== 'stream')
        return;
    if (session.provider !== 'qemu' || session.qemu_profile === 'regression')
        return;
    const args = [...getDesktopActivateArgs(), '--activate-desktop', '--session', session.id];
    await execFileAsync(activateBin, args);
}
async function guestRequest(state, path, init) {
...
async function guestJson(state, path, init) {
    const response = await fetch(`${state.guestRuntimeUrl}${path}`, {
        headers: { 'content-type': 'application/json', ...(init?.headers ?? {}) },
        ...init,
    });
...
async function getGuestSession(state, sessionId) {
    const payload = await guestJson(state, `/api/sessions/${sessionId}`);
    return payload.session;
}
async function appendReviewEvent(state, sessionId, payload) {
    try {
        await guestJson(state, `/api/sessions/${sessionId}/review-events`, { method: 'POST', body: JSON.stringify(payload) });
    }
    catch {
        // Review capture should never break the operator path.
    }
}
async function recordControlPlaneAction(state, sessionId, action, receipt) {
...
async function ensureRemoteChromium() {
    try {
...
async function ensureBrowser(state, sessionId) {
    ...
    if (browserBackendPreference === 'remote-cdp') {
        ...
        const browserState = { context, page, browserName: 'chromium', connectionMode: 'remote-cdp' };
    ...
    const page = context.pages()[0] ?? await context.newPage();
    const browserState = { context, page, browserName: 'firefox', connectionMode: 'display' };
    state.browserStates.set(sessionId, browserState);
    return browserState;
}
async function closeBrowser(state, sessionId) {
...
function hasVisibleWindow(observation) {
    const direct = observation?.active_window;
    if (typeof direct === 'string')
        return direct.trim().length > 0;
    if (direct && typeof direct === 'object' && typeof direct.title === 'string') {
        return (direct.title ?? '').trim().length > 0;
    }
    const summary = observation?.summary;
    if (summary && typeof summary === 'object' && typeof summary.active_window === 'string') {
        return ((summary.active_window) ?? '').trim().length > 0;
    }
    return false;
}
function unsupportedBrowserReceipt(action, message) {
    return {
        status: 'error',
...
async function handleBrowserAction(state, sessionId, action) {
    if (!playwrightEnabled) {
        if (action.kind === 'browser_open') {
            let receipt = await guestJson(state, `/api/sessions/${sessionId}/actions`, { method: 'POST', body: JSON.stringify(toGuestAction(action)) });
            try {
                const session = await getGuestSession(state, sessionId);
                const observation = await guestJson(state, `/api/sessions/${sessionId}/observation`);
            ...
            if (receipt.status === 'ok') {
                ...
                if (snapshot)
                    state.browserSnapshots.set(sessionId, snapshot);
            }
            pushHistory(state, sessionId, { action: action, receipt: receipt, source: 'browser-open-fallback' });
        ...
        if (action.kind === 'browser_click' && typeof action.x === 'number' && typeof action.y === 'number') {
            const receipt = await guestJson(state, `/api/sessions/${sessionId}/actions`, { method: 'POST', body: JSON.stringify(toGuestAction({ kind: 'mouse_click', button: action.button, x: action.x, y: action.y, taskId: action.taskId })) });
            pushHistory(state, sessionId, { action: action, receipt: receipt, source: 'browser-coordinate-fallback' });
            attachReceiptToTask(state, action.taskId, receipt);
            return receipt;
        }
        if (action.kind === 'browser_type' && !action.selector) {
            const receipt = await guestJson(state, `/api/sessions/${sessionId}/actions`, { method: 'POST', body: JSON.stringify(toGuestAction({ kind: 'type_text', text: action.text, taskId: action.taskId })) });
            pushHistory(state, sessionId, { action: action, receipt: receipt, source: 'browser-coordinate-fallback' });
            attachReceiptToTask(state, action.taskId, receipt);
            return receipt;
        }
        if (action.kind === 'browser_screenshot') {
            ...
            pushHistory(state, sessionId, { action: action, receipt: receipt, source: 'browser-screenshot-fallback' });
            attachReceiptToTask(state, action.taskId, receipt);
            await recordControlPlaneAction(state, sessionId, action, receipt);
            return receipt;
        }
        if (action.kind === 'browser_get_dom') {
            ...
            if (snapshot?.lastHtml) {
                ...
                pushHistory(state, sessionId, { action: action, receipt: receipt, source: 'browser-dom-fetch-fallback' });
                attachReceiptToTask(state, action.taskId, receipt);
                await recordControlPlaneAction(state, sessionId, action, receipt);
                return receipt;
            }
        ...
        pushHistory(state, sessionId, { action: action, receipt: receipt, source: 'browser-disabled' });
        attachReceiptToTask(state, action.taskId, receipt);
        await recordControlPlaneAction(state, sessionId, action, receipt);
        return receipt;
    }
    ...
    switch (action.kind) {
        ...
        case 'browser_click':
            if (action.selector) {
                ...
                const receipt = await guestJson(state, `/api/sessions/${sessionId}/actions`, { method: 'POST', body: JSON.stringify(payload) });
                pushHistory(state, sessionId, { action: action, receipt: receipt, source: 'browser-fallback' });
                return receipt;
            }
            else {
    ...
    pushHistory(state, sessionId, { action: action, receipt: receipt, source: 'browser-adapter' });
    attachReceiptToTask(state, action.taskId, receipt);
    await recordControlPlaneAction(state, sessionId, action, receipt);
    return receipt;
}
    ...
    return async (req, res) => {
        try {
            ...
            if (req.method === 'GET' && url.pathname === '/api/health') {
                const health = await guestJson(state, '/health');
            ...
            if (req.method === 'GET' && url.pathname === '/api/sessions') {
                ...
                if (upstream.status === 200 && Array.isArray(upstream.payload?.sessions)) {
                    json(res, 200, {
                        ...upstream.payload,
                        sessions: upstream.payload.sessions.map((session) => withLiveDesktopView(session)),
                    });
                }
                else {
                    json(res, upstream.status, upstream.payload);
            ...
            if (req.method === 'POST' && url.pathname === '/api/sessions') {
                ...
                if (upstream.status === 201 && upstream.payload?.session) {
                    ...
                    await maybeActivateDesktop(upstream.payload.session).catch(() => undefined);
                }
                json(res, upstream.status, upstream.payload);
                return;
            }
            ...
            if (sessionMatch && req.method === 'GET') {
                ...
                if (upstream.status === 200) {
                    json(res, 200, {
                        ...upstream.payload,
                        session: withLiveDesktopView(upstream.payload.session),
            ...
            if (liveViewMatch && ['GET', 'HEAD'].includes(req.method ?? 'GET')) {
                ...
                const rewritten = rewriteLiveViewBody(sessionId, proxyPath, upstream.headers.get('content-type'), body);
            ...
            if (actionsMatch && req.method === 'POST') {
                ...
                if (action.kind.startsWith('browser_')) {
                    receipt = await handleBrowserAction(state, sessionId, action);
                }
                else {
                    const upstream = await guestRequest(state, `/api/sessions/${sessionId}/actions`, {
                        method: 'POST',
                    ...
                    if (upstream.status === 200) {
                        receipt = upstream.payload;
                        pushHistory(state, sessionId, { action: action, receipt: receipt, source: 'guest-runtime' });
                        attachReceiptToTask(state, action.taskId, receipt);
...
async function handleLiveViewUpgrade(state, req, socket, head) {
    const url = new URL(req.url ?? '/', 'http://127.0.0.1');
    ...
    const upstreamBase = new URL(liveDesktopView.debug_url);
    const proxyPath = buildLiveViewProxyPath(url.pathname, sessionId) ?? '/';
    const upstreamSocket = connectSocket(Number(upstreamBase.port || (upstreamBase.protocol === 'https:' ? 443 : 80)), upstreamBase.hostname);
    ...
    upstreamSocket.on('error', () => {
        if (!socket.destroyed)
            writeUpgradeError(socket, 502, 'failed to reach live desktop upstream');
    });
